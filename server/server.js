...
import { fetchKalimatiPrices, clearPriceCache, getCacheStatus } from './utils/kalimatiService.js';
...
// API endpoint to get all market prices (with real-time Kalimati data)
app.get('/api/market-prices', async (req, res) => {
  try {
    // Try to fetch real-time prices from Kalimati
    const kalimatiData = await fetchKalimatiPrices();
    
    if (kalimatiData && kalimatiData.prices && kalimatiData.prices.length > 0) {
      return res.json({
        success: true,
        source: 'kalimati-realtime',
        data: kalimatiData.prices,
        cached: kalimatiData.cached,
        fetchedAt: kalimatiData.timestamp,
        count: kalimatiData.prices.length
      });
    }
    
    // Fallback to local static data if real-time fetch fails
    console.log('Falling back to static market data');
    ...
    return res.json({
      ...
      note: 'Real-time data unavailable, showing static data'
    });
  } catch (error) {
    console.error('Error in market-prices endpoint:', error);
    
    // Final fallback to static data - NEVER return error status
    try {
      const marketData = await parseMarketData();
      return res.json({
        success: true,
        ...
        error: error.message,
        count: marketData.length,
        note: 'Using fallback static data due to error'
      });
    } catch (fallbackError) {
...
app.get('/api/market-prices/search', async (req, res) => {
  try {
    ...
    if (kalimatiData && kalimatiData.prices && kalimatiData.prices.length > 0) {
      searchData = kalimatiData.prices;
    } else {
      // Fallback to static data
      searchData = await parseMarketData();
    }
    ...
    try {
      ...
      const filteredData = query ? 
        marketData.filter(item => item.commodity.toLowerCase().includes(query)) : 
        marketData;
      ...
      return res.json({
        ...
        data: [],
        count: 0,
        error: 'Unable to search market prices'
      });
    }
  }
});
...
// API endpoint to refresh market prices and clear cache
app.post('/api/market-prices/refresh', async (req, res) => {
  try {
    clearPriceCache();
    const kalimatiData = await fetchKalimatiPrices();
    
    if (kalimatiData && kalimatiData.prices.length > 0) {
      res.json({
        success: true,
        message: 'Market prices refreshed successfully',
        source: 'kalimati',
        itemCount: kalimatiData.prices.length,
        fetchedAt: kalimatiData.timestamp
      });
    } else {
      res.json({
        success: true,
        message: 'Cache cleared. Using fallback to static data',
        source: 'static'
      });
    }
  } catch (error) {
...
app.get('/api/market-prices/cache-status', (req, res) => {
...
// API endpoint to get health and API info
app.get('/api/health', async (req, res) => {
  try {
    const cacheStatus = getCacheStatus();
    const kalimatiStatus = await fetchKalimatiPrices().then(data => ({
      available: !!data && !!data.prices.length,
      itemCount: data?.prices?.length || 0,
